...
public class codeitems {
    ...
    // 定义物品组 - 数据碎片组
    public static final ItemGroup Data = FabricItemGroup.builder().icon(() -> new ItemStack(Data_UL)).displayName(Text.translatable("itemGroup.code.data")).entries((context, entries) -> {
        entries.add(Data_UL);
        entries.add(Data_1B);
        entries.add(Data_4B);
